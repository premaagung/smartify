...
type CoursePageClientProps = {
  ...
  progressMap: Record<string, { score: number; total: number }>;
...
const CoursePageClient: React.FC<CoursePageClientProps> = ({
  ...
  // Local state so sidebar updates immediately after quiz submit (no full page reload)
  ...
  // Called by QuizCards after a successful save
  const handleChapterComplete = useCallback(() => {
    setLocalCompletedIds((prev) => new Set([...prev, chapter.id]));
  }, [chapter.id]);
      ...
      {isMobile ? (
        ...
        >
          <CourseSideBar
            ...
            completedChapterIds={Array.from(localCompletedIds)}
      ...
      <div className={`flex-1 px-4 md:px-8 ${isMobile ? "pt-20" : "pt-0"}`}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex-1">
            <QuizCards chapter={chapter} onComplete={handleChapterComplete} />
          </div>
        </div>
